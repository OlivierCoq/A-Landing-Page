<?php   
    if(isset($_POST) && !empty($_POST)){
            ...
            if( 
                isset($_POST['adm-first'], $_POST['adm-last']) &&
                strlen($_POST['adm-first']) > 1 &&
                strlen($_POST['adm-last']) > 1 &&
				preg_match('/^[a-zA-Z\s]+$/', $_POST['adm-first']) &&
				preg_match('/^[a-zA-Z\s]+$/', $_POST['adm-last']) &&
				preg_match('/^(?!(?:(?:\x22?\x5C[\x00-\x7E]\x22?)|(?:\x22?[^\x5C\x22]\x22?)){255,})(?!(?:(?:\x22?\x5C[\x00-\x7E]\x22?)|(?:\x22?[^\x5C\x22]\x22?)){65,}@)(?:(?:[\x21\x23-\x27\x2A\x2B\x2D\x2F-\x39\x3D\x3F\x5E-\x7E]+)|(?:\x22(?:[\x01-\x08\x0B\x0C\x0E-\x1F\x21\x23-\x5B\x5D-\x7F]|(?:\x5C[\x00-\x7F]))*\x22))(?:\.(?:(?:[\x21\x23-\x27\x2A\x2B\x2D\x2F-\x39\x3D\x3F\x5E-\x7E]+)|(?:\x22(?:[\x01-\x08\x0B\x0C\x0E-\x1F\x21\x23-\x5B\x5D-\x7F]|(?:\x5C[\x00-\x7F]))*\x22)))*@(?:(?:(?!.*[^.]{64,})(?:(?:(?:xn--)?[a-z0-9]+(?:-[a-z0-9]+)*\.){1,126}){1,}(?:(?:[a-z][a-z0-9]*)|(?:(?:xn--)[a-z0-9]+))(?:-[a-z0-9]+)*)|(?:\[(?:(?:IPv6:(?:(?:[a-f0-9]{1,4}(?::[a-f0-9]{1,4}){7})|(?:(?!(?:.*[a-f0-9][:\]]){7,})(?:[a-f0-9]{1,4}(?::[a-f0-9]{1,4}){0,5})?::(?:[a-f0-9]{1,4}(?::[a-f0-9]{1,4}){0,5})?)))|(?:(?:IPv6:(?:(?:[a-f0-9]{1,4}(?::[a-f0-9]{1,4}){5}:)|(?:(?!(?:.*[a-f0-9]:){5,})(?:[a-f0-9]{1,4}(?::[a-f0-9]{1,4}){0,3})?::(?:[a-f0-9]{1,4}(?::[a-f0-9]{1,4}){0,3}:)?)))?(?:(?:25[0-5])|(?:2[0-4][0-9])|(?:1[0-9]{2})|(?:[1-9]?[0-9]))(?:\.(?:(?:25[0-5])|(?:2[0-4][0-9])|(?:1[0-9]{2})|(?:[1-9]?[0-9]))){3}))\]))$/iD', $_POST['adm-email']) &&
                preg_match("/^[0-9]{3}-[0-9]{3}-[0-9]{4}$/", $_POST['adm-phone']) &&
                filter_var($_POST['currentPageURL'], FILTER_VALIDATE_URL) !== false &&
                strpos($_POST['currentPageURL'], 'detoxlandingpage.com') !== false
            ) {
                        
            $admFirst =  $_POST['adm-first'];
            $admLast = $_POST['adm-last'];
            $admEmail= $_POST['adm-email'];
            $admPhone = $_POST['adm-phone']; 
            $admMessage = $_POST['adm-message'];
            $admFacility = $_POST['adm-facility'];
            ...
            $to = "contact@oliviercoq.online.com"; /*contact and oliverc*/
